...
class MainView(tk.Tk):
        ...
        try:
            ...
            if reinicio:
                try:
                    ...
                    pass
                # También crear sesión de caja del nuevo día
                try:
                    self.sale_use_case.get_or_create_today_cash_session()
                except Exception:
                    pass
    ...
    def _open_business_popup(self):
        ...
        def build_content(parent):
            ...
            tk.Label(parent,
                     text="Estos datos aparecen en la barra de título, en Pagos, Inventario y tickets.",
    ...
    def _configure_backup(self):
        ...
        def guardar():
            ...
            pop.destroy()
            MD.show_info("✅ Configuración de respaldo guardada.", "Listo", parent=self)
    ...
    def export_inventory_excel(self):
        ...
        if not products:
            MD.show_info("No hay productos en el inventario.", "Sin datos", parent=self)
            return
        ...
        for p in products:
            ...
            ws.append([
                ...
                getattr(p, "rounded_price", p.price) or p.price,
        ...
        for col, w in zip("ABCDEFGHIJK", [8, 35, 15, 20, 12, 10, 12, 14, 10, 14, 14]):
            ws.column_dimensions[col].width = w

        try:
            wb.save(arch)
            MD.show_info(f"✅ Inventario exportado:\n\n{arch}\n\n"
    ...
    def export_price_history(self):
        """Exporta el historial de precios en TXT, Excel o PDF."""
        pop = tk.Toplevel(self)
        ...
        tk.Label(pop, text="Elige el formato de exportación:",
                 font=("Arial", 11), bg=bg, fg=fg).pack(pady=8)
        ...
        def exportar(formato):
            try:
                ...
                data = []
            if not data:
                MD.show_info("No hay historial de precios registrado.",
            ...
            if formato == "txt":
                arch = filedialog.asksaveasfilename(
                    ...
                    filetypes=[("Texto", "*.txt")], parent=pop)
                if not arch:
                    return
                try:
                    with open(arch, "w", encoding="utf-8") as f:
                        ...
                        for r in data:
                            ...
                            f.write(f"   Real: ${r['old_price']:,.0f} → ${r['new_price']:,.0f}   |   "
                                    f"Redondeado: ${r['old_rounded_price']:,.0f} → ${r['new_rounded_price']:,.0f}\n".replace(",", "."))
                            f.write("\n")
                    MD.show_info(f"Guardado:\n{arch}", "Listo", parent=pop)
                except Exception as e:
                    MD.show_error(f"Error: {e}", "Error", parent=pop)
            elif formato == "excel":
                try:
                    ...
                    MD.show_error("Falta openpyxl.", "Error", parent=pop)
                    return
                arch = filedialog.asksaveasfilename(
                    defaultextension=".xlsx",
                    initialfile=f"historial_precios_{hoy}.xlsx",
                    filetypes=[("Excel", "*.xlsx")], parent=pop)
                if not arch:
                    return
                wb = openpyxl.Workbook()
                ws = wb.active
                ws.title = "Historial Precios"
                ws.append(["Fecha", "Producto", "Código",
                           "Precio real anterior", "Precio real nuevo",
                           "Cambio real",
                           "Redondeado anterior", "Redondeado nuevo",
                           "Cambio redondeado"])
                for c in ws[1]:
                    c.font = Font(bold=True, color="FFFFFF")
                    c.fill = PatternFill("solid", fgColor="305496")
                    c.alignment = Alignment(horizontal="center")
                for r in data:
                    cambio_real = r["new_price"] - r["old_price"]
                    cambio_red = r["new_rounded_price"] - r["old_rounded_price"]
                    ws.append([
                        r["date"], r["product_name"], r["product_barcode"],
                        r["old_price"], r["new_price"], cambio_real,
                        r["old_rounded_price"], r["new_rounded_price"], cambio_red
                    ])
                for col, w in zip("ABCDEFGHI", [20, 30, 20, 18, 18, 15, 18, 18, 15]):
                    ws.column_dimensions[col].width = w
                try:
                    wb.save(arch)
                    MD.show_info(f"Guardado:\n{arch}", "Listo", parent=pop)
                ...
                try:
                    ...
                    MD.show_error("Falta reportlab.", "Error", parent=pop)
                    return
                arch = filedialog.asksaveasfilename(
                    defaultextension=".pdf",
                    initialfile=f"historial_precios_{hoy}.pdf",
                    filetypes=[("PDF", "*.pdf")], parent=pop)
                if not arch:
                    return
                try:
                    c = canvas.Canvas(arch, pagesize=letter)
                    w, h = letter
                    c.setFont("Helvetica-Bold", 16)
                    c.drawCentredString(w / 2, h - 2 * cm, "HISTORIAL DE PRECIOS")
                    c.setFont("Helvetica", 10)
                    c.drawCentredString(w / 2, h - 2.6 * cm,
                                        f"Generado: {datetime.now().strftime('%d/%m/%Y %H:%M')}")
                    y = h - 3.5 * cm
                    c.setFont("Helvetica-Bold", 10)
                    c.drawString(1.5 * cm, y, "Fecha")
                    c.drawString(4.5 * cm, y, "Producto")
                    c.drawString(10 * cm, y, "Real")
                    c.drawString(14 * cm, y, "Redondeado")
                    y -= 0.4 * cm
                    ...
                    for r in data:
                        if y < 2.5 * cm:
                            ...
                            y = h - 2 * cm
                            c.setFont("Helvetica", 9)
                        nombre = (r["product_name"] or "")[:40]
                        c.drawString(1.5 * cm, y, r["date"][:16])
                        c.drawString(4.5 * cm, y, nombre)
                        c.drawString(10 * cm, y,
                                     f"${r['old_price']:,.0f}→${r['new_price']:,.0f}".replace(",", "."))
                        c.drawString(14 * cm, y,
                                     f"${r['old_rounded_price']:,.0f}→${r['new_rounded_price']:,.0f}".replace(",", "."))
                        y -= 0.5 * cm
                    c.save()
                    MD.show_info(f"Guardado:\n{arch}", "Listo", parent=pop)
                except Exception as e:
        ...
        bf = tk.Frame(pop, bg=bg)
        bf.pack(pady=15)
        ttk.Button(bf, text="📄 TXT", command=lambda: exportar("txt"),
                   bootstyle="secondary", width=10).pack(side="left", padx=5)
        ttk.Button(bf, text="📊 Excel", command=lambda: exportar("excel"),
                   style="DarkGreen.TButton", width=10).pack(side="left", padx=5)
        ttk.Button(bf, text="📕 PDF", command=lambda: exportar("pdf"),
                   bootstyle="danger", width=10).pack(side="left", padx=5)

        ttk.Button(pop, text="Cancelar", command=pop.destroy).pack(pady=8)
    ...
    def _open_cash_movement(self):
        """Registrar retiro, gasto o ingreso manual en caja."""
        pop = tk.Toplevel(self)
        ...
        tk.Label(pop, text="Monto:", font=("Arial", 11), bg=bg, fg=fg).pack(pady=(10, 3))
        ...
        def guardar():
            ...
            try:
                ...
                pop.destroy()
                MD.show_info(f"✅ {tipo_var.get().capitalize()} de ${monto:,.0f} registrado.".replace(",", "."),
                             "Listo", parent=self)
    ...
    def reset_sales_counter(self):
        ...
        try:
            ...
            MD.show_info("✅ Contador reiniciado.\nLa próxima venta será #01.",
                         "Listo", parent=self)
        except Exception as e:
    ...
    # =========================================================
    # CIERRE DE CAJA (FASE 6 - CON CUADRE COMPLETO)
    # =========================================================
    def show_cash_closing(self):
        ...
        def generar_reporte():
            ...
            lineas.append(f"💵 Efectivo inicial:          ${data.get('initial_cash', 0):,.0f}".replace(",", "."))
            lineas.append(f"💰 Ventas en efectivo:        ${data['metodos'].get('Efectivo', {}).get('total', 0):,.0f}".replace(",", "."))
            lineas.append(f"💵 Abonos en efectivo:        ${data['total_abonos']:,.0f}".replace(",", "."))
            lineas.append(f"➕ Ingresos manuales:         ${data.get('ingresos', 0):,.0f}".replace(",", "."))
            lineas.append(f"➖ Retiros:                  -${data.get('retiros', 0):,.0f}".replace(",", "."))
            lineas.append(f"➖ Gastos:                   -${data.get('gastos', 0):,.0f}".replace(",", "."))
            lineas.append("")
            ...
            if data.get("counted_cash", 0) > 0:
                ...
                lineas.append(f"💵 Efectivo contado:          ${data['counted_cash']:,.0f}".replace(",", "."))
                lineas.append(f"📊 Diferencia:               {signo}${diff:,.0f}  ({estado})".replace(",", "."))
            else:
        ...
        def contar_efectivo():
            ...
            summary = self.sale_use_case.get_cash_session_summary(session_id=session["session_id"])
            ...
            def guardar():
                ...
                try:
                    ...
                    pop.destroy()
                    generar_reporte()
                    MD.show_info("✅ Efectivo contado registrado.", "Listo", parent=win)
                except Exception as ex:
                    MD.show_error(f"Error: {ex}", "Error", parent=pop)
    ...
    def show_sales_summary(self):
        ...
        def ver_detalle(event=None):
            ...
            # Botón imprimir ticket de la venta seleccionada
            ...
            ttk.Button(bf2, text="Cerrar",
                       command=lambda: [det.destroy(), restaurar_foco()]).pack(side="right", padx=5)
    ...
    # =========================================================
    # IMPRIMIR TICKET DE UNA VENTA (FASE 6)
    # =========================================================
    def _print_ticket_for_sale(self, sale_id, parent=None):
        """Genera el PDF del ticket para una venta."""
        try:
    ...
    def generate_monthly_report(self):
        ...
        ws.append(["Venta #", "Fecha", "Cliente", "Método", "Fiado", "Producto",
                   "Código", "Cantidad", "Unidad", "P. Unit.", "Subtotal Item",
        ...
        for col, w in zip("ABCDEFGHIJKLM",
                          [10, 20, 25, 15, 8, 30, 20, 10, 10, 12, 12, 12, 12]):
            ws.column_dimensions[col].width = w
        try:
            wb.save(arch)
            MD.show_info(f"Guardado:\n{arch}", "Listo", parent=self)
